...
class FirebaseOrderRepository : OrderRepository {
    ...
    override suspend fun getOrders(userId: String): Result<List<Order>> {
        return try {
            Log.d("FirebaseOrderRepo", "Buscando pedidos para userId: $userId")

            val querySnapshot = ordersCollection
                .where { "userId" equalTo userId }
                .get()

            Log.d("FirebaseOrderRepo", "Encontrados ${querySnapshot.documents.size} documentos")

            val orders = querySnapshot.documents.mapNotNull { doc ->
                try {
                    Log.d("FirebaseOrderRepo", "Parseando documento: ${doc.id}")

                    // Tentar obter items como lista de OrderItemData
                    val itemsData = try {
                        doc.get<List<OrderItemData>>("items")
                    } catch (e: Exception) {
                        Log.e("FirebaseOrderRepo", "Erro ao deserializar items: ${e.message}")
                        emptyList()
                    }

                    Log.d("FirebaseOrderRepo", "Items encontrados: ${itemsData.size}")
                    ...
                    Log.e("FirebaseOrderRepo", "Erro ao parsear pedido ${doc.id}: ${e.message}", e)
                    null
                }
            }.sortedByDescending { it.createdAt }

            Log.d("FirebaseOrderRepo", "Total de pedidos parseados: ${orders.size}")
            Result.success(orders)
        } catch (e: Exception) {
            Log.e("FirebaseOrderRepo", "Erro ao buscar pedidos: ${e.message}", e)
            Result.failure(e)
        }
    }
    ...
    override suspend fun getOrderById(orderId: String): Result<Order> {
        return try {
            ...
            Log.e("FirebaseOrderRepo", "Erro ao buscar pedido: ${e.message}")
            Result.failure(e)
        }
    }
}
